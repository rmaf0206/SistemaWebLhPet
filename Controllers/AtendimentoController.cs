...
namespace LH_PET_WEB.Controllers
{
    ...
    public class AtendimentoController : Controller
    {
        ...
        public async Task<IActionResult> Prontuario(int agendamentoId)
        {
            ...
            var novoAtendimento = new Atendimento { AgendamentoId = agendamento.Id, Agendamento = agendamento };
        ...
        public async Task<IActionResult> SalvarProntuario(Atendimento model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (agendamento != null) { agendamento.Status = "Concluído"; _contexto.Agendamentos.Update(agendamento); }
                await _contexto.SaveChangesAsync();
        ...
        public async Task<IActionResult> Historico(string pesquisa)
        {
            var query = _contexto.Atendimentos
                .Include(a => a.Agendamento).ThenInclude(ag => ag!.Pet).ThenInclude(p => p!.Cliente).AsQueryable();
        ...
        public async Task<IActionResult> NovoAgendamento()
        {
            ViewBag.PetsLista = await _contexto.Pets
                .Include(p => p.Cliente)
                .Select(p => new { Id = p.Id, NomeExibicao = $"{p.Nome} (Tutor: {p.Cliente!.Nome})" })
                .ToListAsync();
